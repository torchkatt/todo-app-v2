    ...
    <div className="pb-24 bg-brand-bg min-h-screen font-sans">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-0.5">
              <button onClick={() => navigate('/explore')} aria-label="Ubicación" className="p-2.5 rounded-xl hover:bg-gray-100 transition-colors text-text-secondary">
                <MapPin size={20} />
              </button>
              <button aria-label="Notificaciones" className="p-2.5 rounded-xl hover:bg-gray-100 transition-colors text-text-secondary">
                <Bell size={20} />
              </button>
              <button onClick={() => navigate('/cart')} aria-label="Carrito" className="p-2.5 rounded-xl hover:bg-gray-100 transition-colors text-text-secondary relative">
                <ShoppingBag size={20} />
                {totalItems > 0 && (
              ...
              <button onClick={() => navigate(isAuthenticated ? '/profile' : '/login')} aria-label="Perfil" className="p-2.5 rounded-xl hover:bg-gray-100 transition-colors text-text-secondary relative">
                <User size={20} />
      ...
      <main className="max-w-6xl mx-auto px-4">
        ...
        <section className="relative mt-4 mb-6 overflow-hidden rounded-2xl bg-gradient-to-br from-violet-600 via-indigo-600 to-blue-600 p-6 md:p-8 shadow-lg shadow-purple-500/20">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-xs font-bold text-white mb-4">
              <Sparkles size={12} /> Beta — ¡Sé de los primeros!
            </div>
            <h2 className="text-2xl md:text-3xl font-black text-white leading-tight mb-2">
              Todo lo que necesitas, <br />
              <span className="text-amber-200">en un solo lugar</span>
            </h2>
            <p className="text-sm md:text-base text-violet-200 mb-5 max-w-lg">
              Compra productos, reserva servicios, descarga contenido digital. Conecta con negocios locales.
            </p>
        ...
        <div className="grid grid-cols-3 gap-2 mb-8">
          {[
            { icon: TrendingUp, text: '0% comisión', sub: 'Primer mes' },
            { icon: Clock, text: 'Entrega flexible', sub: 'Recoge o envío' },
            { icon: Star, text: 'Verificados', sub: 'Ratings reales' },
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-extrabold text-text-primary tracking-tight">Categorías</h3>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-lg font-extrabold text-text-primary tracking-tight">Destacados</h3>
              <p className="text-xs text-text-muted font-semibold">Lo más popular cerca de ti</p>
            </div>
            <button onClick={() => navigate('/explore')} className="text-xs font-bold text-purple-600 hover:text-purple-700 flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {featuredLoading ? (
              ...
              <div className="col-span-3 text-center py-16 text-text-muted font-semibold">
                No hay productos destacados aún — vuelve pronto
              </div>
        ...
        <section className="mt-8 mb-4">
          <div className="rounded-2xl bg-gradient-to-br from-purple-600 to-indigo-700 p-6 md:p-8 text-center text-white shadow-lg shadow-purple-500/20">
            ...
            <h3 className="text-xl font-extrabold mb-2">¿Tienes algo que vender?</h3>
            <p className="text-sm text-purple-200 mb-5 max-w-md mx-auto">
              Únete a Todo y empieza a vender tus productos, servicios o contenido digital. Primer mes sin comisión.
            </p>
        ...
        <footer className="border-t border-border mt-8 pt-6 pb-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-extrabold text-text-primary">Todo</span>
              <span className="text-xs text-text-muted">— La app que lo tiene todo.</span>
            </div>
            <div className="flex gap-4">
              {['Términos', 'Privacidad', 'Ayuda'].map(l => (
                <a key={l} href="#" className="text-xs font-semibold text-text-muted hover:text-purple-600 transition-colors">{l}</a>
              ))}
            </div>
          </div>
        </footer>
